...
std::unique_ptr<Token> Scanner::Scan() {
  ...
  auto c = ScanNext();
  if (c == '\n')
    return std::make_unique<Token>(NEW_LINE, token_loc, cur_ws_,
                                   cur_token_str_);
  ...
  case '+':
    ...
    return std::make_unique<Token>(str_keyword_map.at(cur_token_str_),
                                   token_loc, cur_ws_, cur_token_str_);
  case '-':
    if (ScanTry('-')) {
      cur_token_str_ += '-';
    } else if (ScanTry('>')) {
  ...
  case '$':
    ParseIdentifier();
    return std::make_unique<Token>(IDENTIFIER, token_loc, cur_ws_,
                                   cur_token_str_);
  case '0' ... '9':
    ParseNum();
    return std::make_unique<Token>(IDENTIFIER, token_loc, cur_ws_,
